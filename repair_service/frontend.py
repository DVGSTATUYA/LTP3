...
class RepairServiceApp:
    ...
    def add_comment(self, request_id, message):
        ...
        comment_data = {
            ...
            "master_id": self.current_user['user_id']
        }
        response = self.session.post(f"{API_URL}/comments/", json=comment_data)
        return response
    ...
    def can_add_comments(self):
        ...
        if not self.current_user:
            return False
        return self.current_user['type'] in ['Мастер', 'Менеджер', 'Менеджер по качеству', 'Оператор']
...
def main():
    ...
    # Главный заголовок
    st.markdown('<h1 class="main-header">🔧 Система учета заявок на ремонт бытовой техники</h1>', unsafe_allow_html=True)
...
def show_new_request_form(app):
    ...
    st.markdown('<h2 class="sub-header">➕ Создание новой заявки</h2>', unsafe_allow_html=True)
...
def show_statistics(app):
    ...
    st.markdown('<h2 class="sub-header">📊 Статистика работы сервиса</h2>', unsafe_allow_html=True)
...
def show_user_management(app):
    ...
    tab1, tab2, tab3 = st.tabs(["📋 Список пользователей", "➕ Добавить пользователя", "✏️ Редактировать пользователя"])
                    ...
                    with col2:
                        ...
                        with st.form("edit_user_form"):
                            ...
                            if delete_btn:
                                ...
                                st.warning("⚠️ Внимание: это действие нельзя отменить!")
...
def show_quality_assessment():
    ...
    # Генерация QR кода
    qr_url = "https://docs.google.com/forms/d/e/1FAIpQLSdhZcExx6LSIXxk0ub55mSu-WIh23WYdGG9HY5EZhLDo7P8eA/viewform"
