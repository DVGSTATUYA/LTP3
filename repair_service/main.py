...
async def create_comment(comment: CommentCreate, master_id: int):
    ...
    # Проверка мастера
    master = db.get_user_by_id(master_id)
    if not master or master['type'] != 'Мастер':
        raise HTTPException(
            status_code=status.HTTP_400_BAD_REQUEST,
            detail="Мастер не найден или не является мастером"
        )
    
    try:
        comment_data = comment.dict()
        comment_data['master_id'] = master_id
